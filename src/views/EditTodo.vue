<template>
    <form v-if="todoItem" class="shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] max-w-sm" @submit.prevent="onSubmit">
    <div class="md:flex md:items-center mb-6">
      ...
      <div class="md:w-2/3">
        <textarea
          ...
          id="inline-full-name" type="text" autocomplete="off" rows="3" v-model="taskName" name="taskName">
        </textarea>
      </div>
    </div>
  ...
  data() {
    ...
    this.todoItem = this.toDoItems.find((todo) => todo.id === Number(this.$route?.params?.id));
    this.taskName = this.todoItem?.todo;
    this.dueDate = new Date(this.todoItem?.dueDate+' Z').toISOString().substring(0, 10);
    console.log('todoList edit',this.todoItem)
  },
  computed: {
    ...mapState([
      'todoList',
